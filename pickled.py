...
# Definizione chiave di encryption Brancamenta
...
def update_switch():
    ...
    if 'index' not in data:
        return jsonify({'success': False, 'message': 'Missing index'})
    ...
    try:
        ...
        try:
            with ConnectHandler(**device) as net_connect:
                ...
                clean_output = "\n".join(
                    full_output.splitlines()[5:-1] 
                    if len(full_output.splitlines()) > 5 
                    else full_output.splitlines()[:-1]
                )
...
# API per il recupero dei log di sistema
...
def get_full_log():
    try:
        ...
        # Recupero i log storici
        if os.path.exists(LOG_DIR):
            for filename in sorted(os.listdir(LOG_DIR), reverse=False):
                if filename.startswith('events.') and filename.endswith('.log') and filename != 'events.log':
                    filepath = os.path.join(LOG_DIR, filename)
                    with open(filepath, 'r') as f:
                        # Filtro solo le righe che iniziano con timestamp e non contengono werkzeug
                        file_lines = [
                        ...
                        if file_lines:
                            ...
                            # Aggiungi separatore e contenuto del file
                            log_lines.append("\n" + "-"*80)
                            log_lines.append("\n>")
                            log_lines.append(f">> Log file: {filename}")
                            log_lines.append("\n>")
                            log_lines.append("-"*80 + "\n")
        
        #Recupero il log corrente
        if os.path.exists(EVENTS_LOG):
            with open(EVENTS_LOG, 'r') as f:
                ...
                if file_lines:
                    ...
                    log_lines.append("\n" + "-"*80)
                    log_lines.append("\n>")
                    log_lines.append(f">> Log file: {EVENTS_LOG}")
                    log_lines.append("\n>")
                    log_lines.append("-"*80 + "\n")
        
        # Inverti le righe per mostrare i log più recenti in alto, poi restituisci il contenuto
        log_lines.reverse()
        full_log = "\n".join(log_lines)
...
"""
@app.route('/export_all_backups', methods=['POST'])
def export_all_backups():
    data = request.get_json()
    switch_index = data.get('index')
    
    switches = get_switches()
    if switch_index < 0 or switch_index >= len(switches):
        return jsonify({'success': False, 'message': 'Invalid switch index'}), 400
    
    switch = switches[switch_index]
    backup_dir = Path(BACKUP_DIR) / switch['hostname']
    
    if not backup_dir.exists() or not any(backup_dir.iterdir()):
        return jsonify({'success': False, 'message': 'No backups available'}), 404
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f"{switch['hostname']}_backups.zip"
    )
"""
...
def export_all_backups():
    ...
    hostname = data.get('hostname', 'backups')

    if not backups:
        return jsonify({'success': False, 'message': 'No backups provided'}), 400

    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        for backup in backups:
            if os.path.exists(backup['path']):
                zip_file.write(backup['path'], backup['filename'])
    ...
    return send_file(
        ...
        mimetype='application/zip',
        as_attachment=True,
        download_name=f"{hostname}_backups.zip"
    )

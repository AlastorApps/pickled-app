...
app.config['WTF_CSRF_SSL_STRICT'] = False  # Disabilita il controllo SSL per CSRF
...
app.config.update(
    SESSION_COOKIE_SECURE=False,  # Disabilita per HTTP
    SESSION_COOKIE_HTTPONLY=True,
    SESSION_COOKIE_SAMESITE='Lax',  # Lax invece di Strict per funzionare tra domini
...
def apply_cors(response):
    # Configurazione CORS per permettere richieste cross-domain
    response.headers['Access-Control-Allow-Origin'] = request.headers.get('Origin', '*')
    response.headers['Access-Control-Allow-Headers'] = 'Content-Type, X-CSRFToken'
    response.headers['Access-Control-Allow-Methods'] = 'GET, POST, PUT, OPTIONS'
    response.headers['Access-Control-Allow-Credentials'] = 'true'
    
    # Imposta il cookie CSRF se non è già presente
    if 'csrf_token' not in request.cookies and session.get('logged_in'):
...
<body>
	...
	function getCSRFToken() {
	    const cookieValue = document.cookie.match(/csrf_token=([^;]+)/);
